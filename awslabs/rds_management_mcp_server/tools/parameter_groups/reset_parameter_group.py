...
async def reset_db_cluster_parameter_group(
    ...
    reset_all_parameters: Annotated[
        bool, Field(description='Whether to reset all parameters in the DB cluster parameter group')
    ] = False,
    parameters: Annotated[
        Optional[List[Dict[str, Any]]], Field(description='List of parameters to reset. Each parameter should include name and optionally apply_method ("immediate" or "pending-reboot")')
    ] = None,
    ...
    try:
        ...
        logger.info(f"Resetting {'all parameters' if reset_all_parameters else 'specified parameters'} in DB cluster parameter group {db_cluster_parameter_group_name}")
        ...
        logger.success(f"Successfully reset parameters in DB cluster parameter group {db_cluster_parameter_group_name}")
        
        result = format_aws_response(response)
        result['message'] = f"Successfully reset {'all parameters' if reset_all_parameters else 'specified parameters'} in DB cluster parameter group {db_cluster_parameter_group_name}"
...
async def reset_db_instance_parameter_group(
    ...
    reset_all_parameters: Annotated[
        bool, Field(description='Whether to reset all parameters in the DB instance parameter group')
    ] = False,
    parameters: Annotated[
        Optional[List[Dict[str, Any]]], Field(description='List of parameters to reset. Each parameter should include name and optionally apply_method ("immediate" or "pending-reboot")')
    ] = None,
    ...
    try:
        ...
        logger.info(f"Resetting {'all parameters' if reset_all_parameters else 'specified parameters'} in DB instance parameter group {db_parameter_group_name}")
        response = await asyncio.to_thread(
        ...
        logger.success(f"Successfully reset parameters in DB instance parameter group {db_parameter_group_name}")
        
        result = format_aws_response(response)
        result['message'] = f"Successfully reset {'all parameters' if reset_all_parameters else 'specified parameters'} in DB instance parameter group {db_parameter_group_name}"
        result['parameters_reset'] = len(response.get('Parameters', []))
